...
@pytest.mark.asyncio
async def test_create_stop_success(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_minimal_fields(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_trip_not_found(client):
    """Test creating a stop for non-existent trip."""
    response = await client.post("/trips/999/stops/", json={
        "name": "Tokyo",
        "start_date": (date.today() + timedelta(days=11)).isoformat(),
        "end_date": (date.today() + timedelta(days=15)).isoformat(),
        "order_index": 0
...
@pytest.mark.asyncio
async def test_create_stop_dates_outside_trip_dates(client, sample_trip):
    ...
    assert "Dates must be within trip dates" in response.json()["detail"]


@pytest.mark.asyncio
async def test_create_stop_end_date_after_trip_end(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_duplicate_order_index(client, sample_trip_with_stop):
    """Test creating a stop with duplicate order_index."""
    trip, existing_stop = sample_trip_with_stop
    response = await client.post(f"/trips/{trip.id}/stops/", json={
        "name": "Kyoto",
...
@pytest.mark.asyncio
async def test_create_stop_empty_name(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_start_date_in_past(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_end_before_start(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_latitude_without_longitude(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_longitude_without_latitude(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_invalid_latitude(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_negative_order_index(client, sample_trip):
...
@pytest.mark.asyncio
async def test_get_all_stops_empty(client, sample_trip):
    """Test getting stops for a trip with no stops."""
    response = await client.get(f"/trips/{sample_trip.id}/stops/")
    assert response.status_code == 200
    assert response.json() == []


@pytest.mark.asyncio
async def test_get_all_stops_with_data(client, sample_trip_with_stop):
    """Test getting all stops for a trip."""
    trip, stop = sample_trip_with_stop
    response = await client.get(f"/trips/{trip.id}/stops/")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
@pytest.mark.asyncio
async def test_get_all_stops_ordered(client, sample_trip_with_multiple_stops):
...
@pytest.mark.asyncio
async def test_get_all_stops_trip_not_found(client):
    """Test getting stops for non-existent trip."""
    response = await client.get("/trips/999/stops/")
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_get_stop_by_id_success(client, sample_trip_with_stop):
    """Test getting a specific stop by ID."""
    trip, stop = sample_trip_with_stop
    response = await client.get(f"/trips/{trip.id}/stops/{stop.id}")
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_get_stop_by_id_not_found(client, sample_trip):
    """Test getting non-existent stop."""
    response = await client.get(f"/trips/{sample_trip.id}/stops/999")
    assert response.status_code == 404
    assert "Stop with ID 999 not found" in response.json()["detail"]


@pytest.mark.asyncio
async def test_get_stop_wrong_trip(client, sample_trip, sample_trip_with_stop):
    """Test getting a stop from the wrong trip."""
    trip1 = sample_trip
    trip2, stop = sample_trip_with_stop
    # Try to get trip2's stop using trip1's ID
    response = await client.get(f"/trips/{trip1.id}/stops/{stop.id}")
    assert response.status_code == 404
...
@pytest.mark.asyncio
async def test_update_stop_name(client, sample_trip_with_stop):
    """Test updating stop name."""
    trip, stop = sample_trip_with_stop
    response = await client.put(f"/trips/{trip.id}/stops/{stop.id}", json={
        "name": "Kyoto"
    })
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_update_stop_dates(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_stop_coordinates(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_stop_not_found(client, sample_trip):
    """Test updating non-existent stop."""
    response = await client.put(f"/trips/{sample_trip.id}/stops/999", json={
        "name": "Ghost Stop"
    })
    assert response.status_code == 404
...
@pytest.mark.asyncio
async def test_update_stop_dates_outside_trip(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_stop_duplicate_order_index(client, sample_trip_with_multiple_stops):
    """Test updating stop to have duplicate order_index."""
    trip, stops = sample_trip_with_multiple_stops
    # Try to update stop[2] to have same order_index as stop[0]
    response = await client.put(f"/trips/{trip.id}/stops/{stops[2].id}", json={
...
@pytest.mark.asyncio
async def test_update_stop_empty_name(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_delete_stop_success(client, sample_trip_with_stop):
    """Test deleting a stop."""
    trip, stop = sample_trip_with_stop
    response = await client.delete(f"/trips/{trip.id}/stops/{stop.id}")
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == stop.id

    # Verify it's deleted
    get_response = await client.get(f"/trips/{trip.id}/stops/{stop.id}")
    assert get_response.status_code == 404


@pytest.mark.asyncio
async def test_delete_stop_not_found(client, sample_trip):
    """Test deleting non-existent stop."""
    response = await client.delete(f"/trips/{sample_trip.id}/stops/999")
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_delete_stop_wrong_trip(client, sample_trip, sample_trip_with_stop):
    """Test deleting a stop using wrong trip ID."""
    trip1 = sample_trip
    trip2, stop = sample_trip_with_stop
    response = await client.delete(f"/trips/{trip1.id}/stops/{stop.id}")
    assert response.status_code == 404
...
@pytest.mark.asyncio
async def test_reorder_stops_success(client, sample_trip_with_multiple_stops):
    ...
    response = await client.put(f"/trips/{trip.id}/stops/reorder", json={
        "stop_ids": new_order
    })
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_reorder_stops_trip_not_found(client):
    """Test reordering stops for non-existent trip."""
    response = await client.put("/trips/999/stops/reorder", json={
        "stop_ids": [1, 2, 3]
    })
    assert response.status_code == 404
...
@pytest.mark.asyncio
async def test_reorder_stops_missing_ids(client, sample_trip_with_multiple_stops):
...
@pytest.mark.asyncio
async def test_reorder_stops_invalid_ids(client, sample_trip_with_multiple_stops):
...
@pytest.mark.asyncio
async def test_reorder_stops_duplicate_ids(client, sample_trip_with_multiple_stops):
...
@pytest.mark.asyncio
async def test_reorder_stops_empty_list(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_create_stop_null_island(client, sample_trip):
...
@pytest.mark.asyncio
async def test_update_stop_to_null_island(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_create_stop_placeholder_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_north_pole_placeholder(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_japan_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_japan_invalid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_israel_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_israel_invalid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_usa_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_usa_alaska_valid(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_usa_hawaii_valid(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_usa_invalid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_no_country_any_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_no_country_still_blocks_null_island(client, sample_trip):
...
@pytest.mark.asyncio
async def test_update_stop_change_country_and_coordinates(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_stop_change_country_but_keep_old_coordinates(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_create_stop_extreme_valid_latitude(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_extreme_valid_longitude(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_case_insensitive_country(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_country_code(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_france_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_germany_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_uk_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_australia_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_canada_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_brazil_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_south_africa_valid_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_france_with_japan_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_australia_with_usa_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_equator_valid(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_prime_meridian_valid(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_null_island_invalid(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_max_latitude_north_pole(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_min_latitude_south_pole(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_max_longitude_international_date_line(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_min_longitude(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_latitude_too_high(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_latitude_too_low(client, sample_trip):
    """Test creating a stop with latitude < -90 - should fail."""
    response = await client.post(f"/trips/{sample_trip.id}/stops/", json={
        "name": "Invalid South",
        "start_date": (date.today() + timedelta(days=11)).isoformat(),
...
@pytest.mark.asyncio
async def test_create_stop_longitude_too_high(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_longitude_too_low(client, sample_trip):
    """Test creating a stop with longitude < -180 - should fail."""
    response = await client.post(f"/trips/{sample_trip.id}/stops/", json={
        "name": "Invalid West",
        "start_date": (date.today() + timedelta(days=11)).isoformat(),
...
@pytest.mark.asyncio
async def test_create_stop_placeholder_1_1(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_high_precision_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_update_stop_valid_coordinates_to_different_country(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_stop_only_coordinates_keep_country(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
@pytest.mark.xfail(reason="Known cleanup issue with many connections", strict=False)
async def test_create_100_stops_performance(client, db_session):
...
@pytest.mark.asyncio
async def test_coordinate_validation_performance(client, sample_trip):
...
@pytest.mark.asyncio
async def test_get_all_stops_performance_with_many_stops(client, db_session):
...
@pytest.mark.asyncio
async def test_create_stop_with_all_edge_case_coordinates(client, sample_trip):
...
@pytest.mark.asyncio
async def test_update_coordinates_and_country_simultaneously_valid(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_coordinates_and_country_simultaneously_invalid(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_create_stop_with_special_characters_in_name(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_with_very_long_name(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_name_too_long(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_multiple_stops_same_day(client, sample_trip):
    """Test creating multiple stops on the same day (1-day overlap allowed)."""
    same_day = (date.today() + timedelta(days=12)).isoformat()

    # First stop - full day
    response1 = await client.post(f"/trips/{sample_trip.id}/stops/", json={
    ...
    response2 = await client.post(f"/trips/{sample_trip.id}/stops/", json={
...
@pytest.mark.asyncio
async def test_update_stop_remove_country(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_stop_remove_coordinates(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_stop_all_fields(client, sample_trip_with_stop):
...
@pytest.mark.asyncio
async def test_update_same_stop_twice_in_succession(client, sample_trip_with_stop):
    """Test updating the same stop twice consecutively."""
    trip, stop = sample_trip_with_stop

    # First update
    response1 = await client.put(f"/trips/{trip.id}/stops/{stop.id}", json={
        "name": "First Update"
    })
    assert response1.status_code == 200

    # Second update
    response2 = await client.put(f"/trips/{trip.id}/stops/{stop.id}", json={
        "name": "Second Update"
    })
...
@pytest.mark.asyncio
async def test_create_stop_with_notes_max_length(client, sample_trip):
...
@pytest.mark.asyncio
async def test_create_stop_with_notes_too_long(client, sample_trip):
